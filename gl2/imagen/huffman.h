...
void muestraHuffman(NodoHuffman<T>* ptr){
	if(ptr == ArbolVacio){
		...
		cout<<"("<<ptr->letra<<":"<<ptr->repeticiones<<" ";
...
NodoHuffman<T>* codifica(Pointer begin,Pointer end){
	...
	for(typename map<T,int>::iterator i = repeticiones.begin();i!=repeticiones.end();i++){
...
template<class T>
void construye_tabla(int profundidad, vector<T> tabla[],NodoHuffman<T>*nodo){
	if(nodo!=ArbolVacio){
		if(es_hoja(nodo)){
...
template<class T>
void construye_diccionario(vector<bool>&profundidad, map<T, vector<bool>>& tabla, NodoHuffman<T>*nodo){
	if(nodo!=ArbolVacio){
		if(es_hoja(nodo)){
